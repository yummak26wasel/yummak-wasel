...
/* ══════════════════════════════════════════════════════
   إضافة جديدة — تجديد قسري للجلسة
   تنادى لمن أي طلب يرجع "JWT expired" (الجلسة انتهت
   قبل ما تتجدد تلقائياً بالخلفية — يصير أحياناً بعد ما
   الموبايل يرمّد التطبيق فترة طويلة). sb-layer.js و
   index.html يستخدمونها لإعادة المحاولة مرة وحدة بس.
   ══════════════════════════════════════════════════════ */
window.__waselRefresh = async function(){
  try{
    const sb = await _client();
    const { data } = await sb.auth.refreshSession();
    if(data && data.session){
      _live = data.session.access_token;
      _save({ uid:data.session.user.id, email:data.session.user.email, token:_live });
      return _live;
    }
  }catch(e){}
  return null;
